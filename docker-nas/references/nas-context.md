...
1. Cargar `.kiro/skills/datasql/SKILL.md` y `docs/services/datasql-guide.md` para instalación/operación, consumidores y creación de bases/roles.
...
4. Crear una base y usuario dedicados con las secciones 5.2–5.5 de la guía: primero el rol, después la base, en llamadas separadas de `psql`.
...
[2026-08-27] Flowise queue mode quedó operativo con main `flowiseai/flowise:latest` y worker `flowiseai/flowise-worker:latest`. Si el worker queda `unhealthy` aunque cree colas, revisar que inicie `node /app/healthcheck/healthcheck.js` en paralelo con `pnpm run start-worker` y que `/healthz` escuche en 5566; no usar la imagen principal con `flowise worker` como variante canónica. Verificar con `svc ps flowise` y `curl -fsS http://127.0.0.1:8100/api/v1/ping` → `pong`. No publicar secretos ni asumir que `latest` fija una versión.
[2026-08-27] En una guía runtime, cada corrección debe alimentar la guía dueña, los derivados afectados, `nas-context.md` si es reutilizable y la skill solo si cambia el proceso. Registrar síntoma, causa, comando completo sin secretos, backup, postcondición y clasificación; distinguir autoalimentación asistida de automatización real si no existe productor/ledger/entrypoint.
[2026-08-27] Para secretos ya existentes, el flujo canónico debe consumirlos localmente desde `$dkco/<servicio>/.env` con variables temporales y `unset`, sin pedir al usuario que los copie o los escriba manualmente cuando pueda automatizarse de forma segura. No confundir un comando que muestra el secreto (`grep | cut`) con uno que lo transporta a una operación local; preferir `$dkco` sobre `/docker` hardcodeado.
```
